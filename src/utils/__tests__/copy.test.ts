...
describe('copy.ts', () => {
    ...
    it('复制内容到系统剪切板', async () => {
        ...
        const spyRemoveChild = vi.spyOn(document.body, 'removeChild').mockImplementation((ele: Node) => ele);

        // mock execCommand 函数，node 本身不支持
        const spyExecCommand = vi.spyOn(document, 'execCommand').mockImplementation((status: string) => !!status);

        // 执行方法
        copyText(mockCopyText);

        const inputEle = document.getElementsByTagName('input')[0];
        const spyInputSelect = vi.spyOn(inputEle, 'select').mockImplementation(() => null);
